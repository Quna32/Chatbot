...
# Create table to log chat sessions
...
# Create a chatbot instance and trainer
...
def construct_api_url(location_name, matched_location):
    ...
    if matched_location:
        # Use latitude and longitude from the existing data rather than making an API call
        lat, lon = matched_location['lat'], matched_location['lon']
        return f"{base_url}lat={lat}&lon={lon}&appid={api_key}&units=metric"
    else:
        # Use the location name provided for the API URL
...
def get_bot_response():
    ...
    if user_input in ['hello', 'hi', 'greetings', 'hey']:
        ...
        conn = sqlite3.connect('pybot.db')
        cursor = conn.cursor()
        cursor.execute("SELECT response FROM responses WHERE query = 'greet'")
        ...
        if data:
            ...
            bot_response = "Hello! How may I assist you?"
        ...
        conn = sqlite3.connect('pybot.db')
        cursor = conn.cursor()
        cursor.execute("SELECT response FROM responses WHERE query = 'weather'")
        data = cursor.fetchone()
        conn.close()
        ...
        if match:
            ...
            if response.status_code == 200:
                ...
                bot_response = "Sorry, I wasn't able to find that location."
        else:
            bot_response = "Please enter a location and I will try to get the weather forecast."
